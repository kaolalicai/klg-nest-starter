...
import { RedisModuleBuilder, RedisService, Redlock, RedlockModule, RedlockService } from '@kalengo/redis'
...
describe('UsersService', () => {
  ...
  describe('decorator mutex', () => {
    ...
    it('should ttl work', async () => {
      ...
      expect(spy).toBeCalledWith(expect.any(Function), {resource: resourceValue, 'ttl': 1000})
    })

    it('should decorator key work', async () => {
      await usersService.decoratorMutexByDecoratorKey()
      expect(spy).toBeCalledWith(expect.any(Function), {resource: 'DecoratorKey', 'ttl': 60000})
    })

    it('should decorator option key work', async () => {
      await usersService.decoratorMutexByDecoratorOptionKey()
      expect(spy).toBeCalledWith(expect.any(Function), {resource: 'DecoratorOptionKey', 'ttl': 1000000})
    })

    it('should LockKey work', async () => {
      await usersService.decoratorMutexByKey('testKey')
      expect(spy).toBeCalledWith(expect.any(Function), {resource: 'testKey', 'ttl': 1000000})
    })
  })
